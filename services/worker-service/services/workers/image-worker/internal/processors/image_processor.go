...
import (
	...
	"github.com/fernandobarroso/workers/image-worker/internal/domain"
)
...
func (p *ImageProcessor) Process(ctx context.Context, msg *queue.Message) error {
	...
	imageMsg, err := domain.NewImageMessage(msg)
	if err != nil {
	...
	case domain.ProcessingTypeResize:
		err = p.processImageResize(ctx, imageMsg)
	case domain.ProcessingTypeFilter:
		err = p.processImageFilter(ctx, imageMsg)
	case domain.ProcessingTypeAnalyze:
		err = p.processImageAnalyze(ctx, imageMsg)
	default:
		p.metrics.RecordProcessingError()
		return domain.ErrInvalidProcessingType
	}
...
func (p *ImageProcessor) processImageResize(ctx context.Context, msg *domain.ImageMessage) error {
...
func (p *ImageProcessor) processImageFilter(ctx context.Context, msg *domain.ImageMessage) error {
...
func (p *ImageProcessor) processImageAnalyze(ctx context.Context, msg *domain.ImageMessage) error {
...
func (p *ImageProcessor) callPythonContainer(ctx context.Context, containerName string, msg *domain.ImageMessage) error {
